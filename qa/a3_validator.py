...
def _canonical_rows(conn: sqlite3.Connection) -> list[dict[str, Any]]:
    source: dict[int, tuple[int | None, str | None]] = {}
    for row in conn.execute(
        """SELECT message_id, source_row_id, source_message_id
           FROM message_source
           ORDER BY message_id,
        ...
        if message_id in source:
            continue
        try:
            source_order = None if row[1] is None else int(row[1])
        except (TypeError, ValueError):
            source_order = None
        source[message_id] = (source_order, None if row[2] is None else str(row[2]))
    ...
    for row in conn.execute(
        ...
        source_order, source_message_id = source.get(message_id, (None, None))
        rows.append(
            {
...
def _expected_order(canonical: list[dict[str, Any]]) -> dict[int, list[dict[str, Any]]]:
...
def _expected_sessions(
    ...
    for conversation_id in sorted(grouped):
        ...
        for index in range(1, len(rows) + 1):
            ...
            if boundary and rows[start:index]:
                sessions.append([row["membership_id"] for row in rows[start:index]])
                start = index
    return sessions
...
def validate_a3_database(path: str | Path) -> dict[str, Any]:
    ...
    try:
        ...
        if fk:
            _issue(issues, "ERROR", "SQLITE_FOREIGN_KEY_ERRORS", f"{len(fk)} foreign-key violation(s)")
        ...
        if run is None:
            _issue(issues, "ERROR", "A3_COMPLETED_RUN_MISSING", "No completed A3 processing_run exists")
            return _finalize(path, issues, checks)
        run_id = int(run["id"])
        ...
        try:
            ...
            config = {}
            _issue(issues, "ERROR", "A3_CONFIG_JSON_INVALID", f"processing_run {run_id} has invalid config_json")
        ...
        if isinstance(gap_seconds, bool) or not isinstance(gap_seconds, int) or gap_seconds <= 0:
            _issue(issues, "ERROR", "A3_SESSION_GAP_INVALID", f"processing_run {run_id} has invalid session_gap_seconds")
            gap_threshold_us = 0
        else:
            gap_threshold_us = gap_seconds * 1_000_000

        canonical = _canonical_rows(conn)
        grouped = _expected_order(canonical)
        canonical_memberships = {row["membership_id"] for row in canonical}
        canonical_messages = {row["message_id"] for row in canonical}
        processed = [dict(row) for row in conn.execute(
        ...
        if canonical_memberships != processed_memberships:
            missing_memberships = canonical_memberships - processed_memberships
            extra_memberships = processed_memberships - canonical_memberships
            _issue(
                issues,
                "ERROR",
                "A3_MEMBERSHIP_RECONCILIATION_FAILED",
                f"missing={len(missing_memberships)}, extra={len(extra_memberships)}",
            )
        if int(run["input_membership_count"]) != len(canonical_memberships):
            _issue(issues, "ERROR", "A3_INPUT_COUNT_MISMATCH", "processing_run input_membership_count differs from current canonical memberships")
        if int(run["output_membership_count"]) != len(processed):
            _issue(issues, "ERROR", "A3_OUTPUT_COUNT_MISMATCH", "processing_run output_membership_count differs from processed_message rows")
        if int(run["canonical_message_count"]) != len(canonical_messages):
            _issue(issues, "ERROR", "A3_CANONICAL_MESSAGE_COUNT_MISMATCH", "processing_run canonical_message_count differs from canonical messages represented")
        ...
        for row in processed:
            ...
            if expected is None:
                continue
            if int(row["message_id"]) != expected["message_id"] or int(row["conversation_id"]) != expected["conversation_id"]:
                identity_mismatches += 1
        checks["processed_identity_mismatches"] = identity_mismatches
        if identity_mismatches:
            _issue(issues, "ERROR", "A3_CANONICAL_IDENTITY_MISMATCH", f"{identity_mismatches} processed membership(s) changed canonical message/conversation identity")
        ...
        for conversation_id, expected_rows in grouped.items():
            actual_rows = sorted(processed_by_conversation.get(conversation_id, []), key=lambda row: int(row["sequence_number"]))
            ...
            if expected_ids != actual_ids or actual_sequences != list(range(1, len(actual_rows) + 1)):
                order_mismatches += 1
        checks["conversation_order_mismatches"] = order_mismatches
        if order_mismatches:
            _issue(issues, "ERROR", "A3_SEQUENCE_ORDER_MISMATCH", f"{order_mismatches} conversation(s) differ from canonical deterministic ordering")

        expected_session_groups = {frozenset(group) for group in _expected_sessions(grouped, gap_threshold_us)} if gap_threshold_us else set()
        actual_session_groups: set[frozenset[int]] = set()
        for row in conn.execute(
            """SELECT session_id, membership_id FROM processed_message
               WHERE processing_run_id=? ORDER BY session_id,sequence_number""",
            (run_id,),
        ):
            session_id = int(row[0])
            found = next((group for group in actual_session_groups if False), None)
        session_members: dict[int, list[int]] = defaultdict(list)
        for row in processed:
            session_members[int(row["session_id"])].append(int(row["membership_id"]))
        ...
        if gap_threshold_us and expected_session_groups != actual_session_groups:
            _issue(issues, "ERROR", "A3_SESSION_PARTITION_MISMATCH", "processed session membership partition differs from independently derived timestamp boundaries")

        session_metadata_errors = 0
        for session_id, memberships in session_members.items():
            metadata = conn.execute(
                """SELECT conversation_id,first_membership_id,last_membership_id,message_count,gap_threshold_us
                   FROM conversation_session WHERE processing_run_id=? AND id=?""",
                (run_id, session_id),
            ).fetchone()
            ordered_memberships = sorted(
                memberships,
                key=lambda membership_id: (
                    int(next(row["sequence_number"] for row in processed if int(row["membership_id"]) == membership_id))
                ),
            )
            if metadata is None:
                session_metadata_errors += 1
                continue
            conversations = {canonical_by_membership[mid]["conversation_id"] for mid in memberships if mid in canonical_by_membership}
            if (
                len(conversations) != 1
                or int(metadata["conversation_id"]) not in conversations
                or int(metadata["first_membership_id"]) != ordered_memberships[0]
                or int(metadata["last_membership_id"]) != ordered_memberships[-1]
        ...
        if session_metadata_errors:
            _issue(issues, "ERROR", "A3_SESSION_METADATA_MISMATCH", f"{session_metadata_errors} persisted session row(s) disagree with assigned memberships")

        expected_run_groups = {frozenset(group) for group in _expected_sender_runs(grouped)}
        sender_members: dict[int, list[int]] = defaultdict(list)
        for row in processed:
            sender_members[int(row["sender_run_id"])].append(int(row["membership_id"]))
        actual_run_groups = {frozenset(values) for values in sender_members.values()}
        checks["expected_sender_run_count"] = len(expected_run_groups)
        checks["actual_sender_run_count"] = len(actual_run_groups)
        if expected_run_groups != actual_run_groups:
            _issue(issues, "ERROR", "A3_SENDER_RUN_PARTITION_MISMATCH", "sender-run partition differs from canonical consecutive sender grouping")

        cross_conversation_threads = int(conn.execute(
            """SELECT COUNT(*) FROM conversation_thread_message ctm
               ...
               WHERE ctm.processing_run_id=? AND mc.conversation_id<>ct.conversation_id""",
            (run_id,),
        ).fetchone()[0])
        checks["cross_conversation_thread_memberships"] = cross_conversation_threads
        if cross_conversation_threads:
            _issue(issues, "ERROR", "A3_THREAD_CROSSES_CONVERSATION", f"{cross_conversation_threads} thread membership(s) cross canonical conversations")

        source_trace_missing = int(conn.execute(
        ...
        if source_trace_missing:
            _issue(issues, "ERROR", "A3_SOURCE_TRACE_MISSING", f"{source_trace_missing} processed membership(s) cannot resolve to an A1 source_record_key")

        attachment_feature_errors = int(conn.execute(
            """SELECT COUNT(*) FROM processed_message pm
               WHERE pm.processing_run_id=? AND (
                   pm.attachment_count <> (
                       SELECT COUNT(*) FROM analysis_attachments aa WHERE aa.message_id=pm.message_id
                   )
                   OR pm.has_attachment <> CASE WHEN EXISTS (
                       SELECT 1 FROM analysis_attachments aa WHERE aa.message_id=pm.message_id
                   ) THEN 1 ELSE 0 END
               )""",
            (run_id,),
        ).fetchone()[0])
        ...
        if attachment_feature_errors:
            _issue(issues, "ERROR", "A3_ATTACHMENT_FEATURE_MISMATCH", f"{attachment_feature_errors} processed membership(s) disagree with A2 attachment occurrences")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Read-only A7 validator for integrated A3 processing")
