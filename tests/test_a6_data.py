...
def test_demo_messages_are_canonical_and_sorted():
    ...
    assert list(frame.columns) == ["message_id", "conversation_id", "contact", "sender", "timestamp", "text"]
...
def test_sqlite_schema_discovery_and_read_only_load(tmp_path):
    ...
    with sqlite3.connect(db_path) as conn:
        conn.execute("CREATE TABLE analytical_messages (id TEXT, thread_id TEXT, chat_name TEXT, author TEXT, sent_at_utc TEXT, body TEXT)")
        conn.execute("INSERT INTO analytical_messages VALUES (?, ?, ?, ?, ?, ?)", ("m1", "c1", "Kontakt", "Roman", "2026-08-01T08:00:00Z", "Ahoj"))
        conn.commit()
    ...
    assert frame.loc[0, "sender"] == "Roman"
