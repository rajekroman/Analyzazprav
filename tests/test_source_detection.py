...
def test_detects_apple_messages_sqlite_by_schema(tmp_path: Path) -> None:
...
def test_detects_imazing_csv_conservatively(tmp_path: Path) -> None:
...
def test_detects_generic_csv_without_claiming_imazing(tmp_path: Path) -> None:
...
def test_txt_detection_requires_explicit_record_mode(tmp_path: Path) -> None:
