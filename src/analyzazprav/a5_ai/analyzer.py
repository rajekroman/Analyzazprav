...
from .result_validation import validate_resolved_result
    ...
    def analyze(
        ...
        if self.cache and not request.force_refresh:
            ...
            try:
                ...
                if cached is not None:
                    validate_resolved_result(cached, context)
            except (ValidationError, ValueError, TypeError, KeyError):
                # Persisted cache is an optimization, never an authority. Any
                # deserialize/evidence mismatch makes this entry unusable and the
                # provider path recomputes a fresh, fully validated result.
                cached = None
            if cached is not None:
                return AnalysisExecution(
                    AnalysisStatus.CACHE_HIT, cached, context_hash
                )
        ...
        try:
            ...
            self._store_failure(
                context_hash,
                request,
                AnalysisStatus.ANALYSIS_TIMEOUT,
                str(exc),
            )
            return AnalysisExecution(
                AnalysisStatus.ANALYSIS_TIMEOUT, None, context_hash, str(exc)
            )
            ...
            return AnalysisExecution(
                AnalysisStatus.MODEL_UNAVAILABLE, None, context_hash, str(exc)
            )
        except ProviderError as exc:
            self._store_failure(
                context_hash,
                request,
                AnalysisStatus.INVALID_OUTPUT,
                str(exc),
            )
            return AnalysisExecution(
                AnalysisStatus.INVALID_OUTPUT, None, context_hash, str(exc)
            )
        ...
        try:
            ...
            repair_prompt = build_repair_prompt(
                user_prompt, raw, str(first_error)
            )
            try:
                repaired_raw = self.provider.analyze(
                    system_prompt=SYSTEM_PROMPT,
            ...
            try:
                ...
                error = (
                    ...
                    f"repair validation: {second_error}"
                )
                self._store_failure(
                    context_hash,
                    request,
        ...
        validate_resolved_result(result, context)
        if self.cache:
            self.cache.put(
        ...
        return AnalysisExecution(
            AnalysisStatus.COMPLETED, result, context_hash
        )
