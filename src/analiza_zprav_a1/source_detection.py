...
def detect_source(path: Path) -> DetectionResult:
    ...
    if suffix == ".jsonl":
        return DetectionResult(
            "generic_message_jsonl", "extension",
            "JSONL records are validated during import",
        )
    if suffix == ".json":
        return DetectionResult(
            "generic_message_json", "extension",
            "JSON structure is validated during import",
        )
    if suffix == ".txt":
        return DetectionResult(
            "generic_message_text", "extension",
